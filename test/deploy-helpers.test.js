...
test('parseDeployArgs: --flag value form', () => {
  assert.deepEqual(
    ...
    { project: 'p', app: 'a', domain: 'd', target: undefined },
  );
});
...
test('parseDeployArgs: bare positionals map to project/app/domain', () => {
  assert.deepEqual(
    ...
    { project: 'myproj', app: 'myapp', domain: 'app.example.com', target: undefined },
  );
});
...
test('parseDeployArgs: domain optional', () => {
  assert.deepEqual(
    ...
    { project: 'p', app: 'a', domain: undefined, target: undefined },
  );
});
...
test('parseDeployArgs: flags win over positionals', () => {
  assert.deepEqual(
    ...
    { project: 'pos1', app: 'flagapp', domain: undefined, target: undefined },
  );
});
...
test('parseDeployArgs: --flag does not swallow a following flag as its value', () => {
  ...
  assert.deepEqual(
    ...
    { project: undefined, app: 'myapp', domain: undefined, target: undefined },
  );
});
...
test('parseDeployArgs: trailing --flag with no value is ignored, not crashed', () => {
  assert.deepEqual(
    ...
    { project: undefined, app: 'a', domain: undefined, target: undefined },
  );
});

test('parseDeployArgs: --target parsed in both value and =value forms', () => {
  assert.equal(parseDeployArgs(['--project', 'p', '--app', 'a', '--target', 'hybrid']).target, 'hybrid');
  assert.equal(parseDeployArgs(['--target=dokploy']).target, 'dokploy');
});

// ---------------------------------------------------------------------------
// HYBRID target: resolveTarget + resolveHybridConvexUrl
// ---------------------------------------------------------------------------
test('resolveTarget: defaults to dokploy, accepts hybrid, lowercases', () => {
  assert.equal(resolveTarget(undefined), 'dokploy');
  assert.equal(resolveTarget('hybrid'), 'hybrid');
  assert.equal(resolveTarget('HYBRID'), 'hybrid');
});

test('resolveTarget: rejects vercel with a pointer to /sc-vercel', () => {
  assert.throws(() => resolveTarget('vercel'), /sc-vercel/);
});

test('resolveTarget: rejects unknown targets', () => {
  assert.throws(() => resolveTarget('flyio'), /unknown --target/);
});

test('resolveHybridConvexUrl: derives cloud URL from a prod deploy key', () => {
  // deriveCloudUrl parses `prod:<name>|<jwt>` → https://<name>.convex.cloud
  assert.equal(
    resolveHybridConvexUrl({ deployKey: 'prod:qualified-jaguar-123|eyJhbGc' }),
    'https://qualified-jaguar-123.convex.cloud',
  );
});

test('resolveHybridConvexUrl: falls back to env URL for a preview key', () => {
  assert.equal(
    resolveHybridConvexUrl({ deployKey: 'preview:branch-x|eyJ', envUrl: '  https://foo.convex.cloud  ' }),
    'https://foo.convex.cloud',
  );
});

test('resolveHybridConvexUrl: null when neither key nor env URL resolves', () => {
  assert.equal(resolveHybridConvexUrl({ deployKey: 'preview:branch-x|eyJ' }), null);
  assert.equal(resolveHybridConvexUrl({}), null);
});
